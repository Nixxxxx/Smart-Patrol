        ...
        <div class="box-body table-responsive no-padding">
          <table class="table table-hover table-bordered text-center">
            ...
            <tr>
              ...
              <td>${members[2*status.index] }<br>[${members[2*status.index+1] }]</td>
              <td>${devices[2*status.index] }<br>[${devices[2*status.index+1] }]</td>
              <td>${locations[2*status.index] }<br>[${locations[2*status.index+1] }]</td>
              <td>${task.startTime }-${task.endTime }</td>
              <td>${task.addTime }</td>
              <td>
              	<c:if test="${task.nowStage==1}" >
              		<span class="text-light-blue">待完成</span>
              	</c:if>
              	<c:if test="${task.nowStage==2}" >
              		<span class="text-green">已完成</span>
              	</c:if>
              	<c:if test="${task.nowStage==3}" >
              		<span class="text-yellow">未按时完成</span>
              	</c:if>
              	<c:if test="${task.nowStage==4}" >
              		<span class="text-red">未完成</span>
              	</c:if>
              </td>
              <td>
                <a data-record-id="${task.recordId }" class="show_detail" href="javascript:void(0)">查看详情</a>
              </td>
              <td>
                <a data-id="${task.id }" class="del" href="javascript:void(0)">
            ...
            if (confirm("确认要删除吗？")) {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if (data.success) {
                            //菜单栏当前选中
                            window.location.href="/Xungeng/patrol/showTaskList";
